...
class TestGetWorkspaceTeams:
    ...
    async def test_get_teams_success_owner(self, client: AsyncClient):
    ...
    async def test_get_teams_success_admin(self, client: AsyncClient):
    ...
    async def test_get_teams_success_manager(self, client: AsyncClient):
    ...
    async def test_get_teams_no_auth(self, client: AsyncClient):
        """401 — без токена."""
        ws = await create_workspace_with_owner(client)
        resp = await client.get(f"{API}/workspaces/{ws['ws_id']}/teams")
        assert resp.status_code == 401

    async def test_get_teams_forbidden_member(self, client: AsyncClient):
