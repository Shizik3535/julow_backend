...
class TestTransferWorkspaceOwnership:
    ...
    async def test_transfer_success_owner(self, client: AsyncClient, workspace_owner):
    ...
    async def test_transfer_no_auth(self, client: AsyncClient, workspace_owner):
        """401 — без токена."""
        ws = workspace_owner
        resp = await client.post(
            f"{API}/workspaces/{ws['ws_id']}/transfer-ownership",
    ...
    async def test_transfer_forbidden_admin(self, client: AsyncClient, workspace_owner):
        """403 — admin (нет ws.*)."""
        ws = workspace_owner
    ...
    async def test_transfer_forbidden_manager(self, client: AsyncClient, workspace_owner):
        """403 — manager."""
        ws = workspace_owner
