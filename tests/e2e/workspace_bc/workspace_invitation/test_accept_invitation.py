...
class TestAcceptWorkspaceInvitation:
    """POST /workspaces/invitations/{invitation_id}/accept — Принять (без проверки permission, только auth)."""

    async def test_accept_success(self, client: AsyncClient, workspace_owner):
        """200 — аутентифицированный пользователь принимает."""
        ws = workspace_owner
    ...
    async def test_accept_no_auth(self, client: AsyncClient):
        """401 — без токена."""
        resp = await client.post(
            f"{API}/workspaces/invitations/00000000-0000-0000-0000-000000000000/accept"
        )
        assert resp.status_code == 401

    async def test_accept_not_found(self, client: AsyncClient):
