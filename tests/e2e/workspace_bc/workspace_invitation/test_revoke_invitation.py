...
class TestRevokeWorkspaceInvitation:
    ...
    async def test_revoke_success_owner(self, client: AsyncClient, workspace_owner):
        """200 — owner отзывает."""
        ws = workspace_owner
    ...
    async def test_revoke_success_admin(self, client: AsyncClient, workspace_owner):
        """200 — admin отзывает."""
        ws = workspace_owner
    ...
    async def test_revoke_no_auth(self, client: AsyncClient, workspace_owner):
        """401 — без токена."""
        ws = workspace_owner
        resp = await client.post(
    ...
    async def test_revoke_forbidden_manager(self, client: AsyncClient, workspace_owner):
        """403 — manager."""
        ws = workspace_owner
    ...
    async def test_revoke_not_found(self, client: AsyncClient, workspace_owner):
