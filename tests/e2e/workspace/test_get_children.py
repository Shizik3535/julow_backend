...
class TestGetChildWorkspaces:
    ...
    async def test_get_children_success_owner(self, client: AsyncClient):
    ...
    async def test_get_children_no_auth(self, client: AsyncClient):
        """401 — без токена."""
        ws = await create_workspace_with_owner(client)
        resp = await client.get(f"{API}/workspaces/{ws['ws_id']}/children")
        assert resp.status_code == 401

    async def test_get_children_forbidden_not_member(self, client: AsyncClient):
        """403 — не участник."""
        ws = await create_workspace_with_owner(client)
