...
class TestSuspendWorkspace:
    ...
    async def test_suspend_success_owner(self, client: AsyncClient):
    ...
    async def test_suspend_success_admin(self, client: AsyncClient):
    ...
    async def test_suspend_no_auth(self, client: AsyncClient):
        """401 — без токена."""
        ws = await create_workspace_with_owner(client)
        resp = await client.post(
            f"{API}/workspaces/{ws['ws_id']}/suspend",
    ...
    async def test_suspend_forbidden_manager(self, client: AsyncClient):
        """403 — manager."""
        ws = await create_workspace_with_owner(client)
    ...
    async def test_suspend_conflict_already_suspended(self, client: AsyncClient):
