...
class TestAddWorkspaceOwner:
    ...
    async def test_add_owner_success(self, client: AsyncClient):
    ...
    async def test_add_owner_no_auth(self, client: AsyncClient):
        """401 — без токена."""
        ws = await create_workspace_with_owner(client)
        resp = await client.post(
            f"{API}/workspaces/{ws['ws_id']}/owners",
    ...
    async def test_add_owner_forbidden_admin(self, client: AsyncClient):
        """403 — admin (нет ws.*)."""
        ws = await create_workspace_with_owner(client)
    ...
    async def test_add_owner_forbidden_manager(self, client: AsyncClient):
        """403 — manager."""
        ws = await create_workspace_with_owner(client)
