...
class TestRestoreWorkspace:
    ...
    async def test_restore_success_owner(self, client: AsyncClient):
    ...
    async def test_restore_success_admin(self, client: AsyncClient):
    ...
    async def test_restore_no_auth(self, client: AsyncClient):
        """401 — без токена."""
        ws = await create_workspace_with_owner(client)
        resp = await client.post(f"{API}/workspaces/{ws['ws_id']}/restore")
        assert resp.status_code == 401

    async def test_restore_forbidden_manager(self, client: AsyncClient):
        """403 — manager."""
        ws = await create_workspace_with_owner(client)
    ...
    async def test_restore_conflict_not_archived(self, client: AsyncClient):
