"""E2E-тесты: PATCH /orgs/{org_id}/roles/{role_id}."""

import uuid

import pytest

from tests.e2e.conftest import API, auth_headers, register_and_login


@pytest.mark.e2e
class TestUpdateRole:
    """Обновление роли."""

    async def _create_custom_role(self, client, owner) -> str:
        """Создаёт кастомную роль и возвращает её ID."""
        resp = await client.post(
            f"{API}/orgs/{owner['org_id']}/roles",
            json={
                "name": "Role To Update",
                "permissions": ["members:read"],
                "scope": "org",
            },
            headers=auth_headers(owner["access_token"])
        )
        assert resp.status_code == 201
        return resp.json()["data"]["id"]

    async def test_update_role_success(self, client, org_with_owner) -> None:
        """200 — обновление кастомной роли."""
        owner = org_with_owner
        role_id = await self._create_custom_role(client, owner)
        resp = await client.patch(
            f"{API}/orgs/{owner['org_id']}/roles/{role_id}",
            json={"permissions": ["members:read", "members:write"], "description": "Updated"},
            headers=auth_headers(owner["access_token"])
        )

        assert resp.status_code == 200

    async def test_update_role_system_forbidden(self, client, org_with_owner) -> None:
        """409 — нельзя обновить системную роль."""
        owner = org_with_owner
        roles_resp = await client.get(
            f"{API}/orgs/{owner['org_id']}/roles",
            headers=auth_headers(owner["access_token"])
        )
        system_roles = [r for r in roles_resp.json()["data"] if r.get("is_system")]
        if not system_roles:
            pytest.skip("Нет системных ролей")

        role_id = system_roles[0]["id"]
        resp = await client.patch(
            f"{API}/orgs/{owner['org_id']}/roles/{role_id}",
            json={"description": "Hacked"},
            headers=auth_headers(owner["access_token"])
        )
        assert resp.status_code == 409

    async def test_update_role_forbidden(self, client, org_with_owner) -> None:
        """403 — не-владелец не может обновить роль."""
        owner = org_with_owner
        role_id = await self._create_custom_role(client, owner)
        other = await register_and_login(client)
        resp = await client.patch(
            f"{API}/orgs/{owner['org_id']}/roles/{role_id}",
            json={"description": "Hacked"},
            headers=auth_headers(other["access_token"])
        )
        assert resp.status_code == 403

    async def test_update_role_not_found(self, client, org_with_owner) -> None:
        """404 — роль не найдена."""
        owner = org_with_owner
        resp = await client.patch(
            f"{API}/orgs/{owner['org_id']}/roles/{uuid.uuid4()}",
            json={"description": "Ghost"},
            headers=auth_headers(owner["access_token"])
        )
        assert resp.status_code == 404

    async def test_update_role_no_auth(self, client) -> None:
        """401 — без токена авторизации."""
        resp = await client.patch(
            f"{API}/orgs/{uuid.uuid4()}/roles/{uuid.uuid4()}",
            json={"description": "NoAuth"}
        )
        assert resp.status_code == 401
