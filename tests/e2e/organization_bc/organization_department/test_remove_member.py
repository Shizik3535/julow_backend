...
class TestRemoveDepartmentMember:
    ...
    async def _setup_dept_with_member(self, client, org_with_owner) -> dict:
    ...
    async def test_remove_department_member_success(self, client, org_with_owner) -> None:
        """200 — удаление участника из подразделения."""
        ctx = await self._setup_dept_with_member(client, org_with_owner)
    ...
    async def test_remove_department_member_forbidden(self, client, org_with_owner) -> None:
    ...
    async def test_remove_department_member_not_found(self, client, org_with_owner) -> None:
