...
class TestGetWorkspaceRoles:
    ...
    async def test_get_roles_success_owner(self, client: AsyncClient):
    ...
    async def test_get_roles_success_admin(self, client: AsyncClient):
    ...
    async def test_get_roles_system_only(self, client: AsyncClient):
    ...
    async def test_get_roles_no_auth(self, client: AsyncClient):
        """401 — без токена."""
        ws = await create_workspace_with_owner(client)
        resp = await client.get(f"{API}/workspaces/{ws['ws_id']}/roles")
        assert resp.status_code == 401

    async def test_get_roles_forbidden_manager(self, client: AsyncClient):
    ...
    async def test_get_roles_forbidden_member(self, client: AsyncClient):
        """403 — member."""
        ws = await create_workspace_with_owner(client)
