...
class TestUnlockAccount:
    ...
    async def test_unlock_account_success(self, client, app) -> None:
        """200 — аккаунт разблокирован (admin-пользователь)."""
        admin = await register_admin_and_login(client, app)
    ...
    async def test_unlock_account_not_found(self, client, app) -> None:
