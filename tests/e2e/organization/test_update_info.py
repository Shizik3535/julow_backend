...
class TestUpdateOrganizationInfo:
    ...
    async def test_update_organization_info_success(self, client) -> None:
    ...
    async def test_update_organization_info_forbidden(self, client) -> None:
    ...
    async def test_update_organization_info_not_found(self, client) -> None:
    ...
    async def test_update_organization_info_validation(self, client) -> None:
        """422 — пустое название."""
        owner = await create_org_with_owner(client)
        resp = await client.patch(
            f"{API}/orgs/{owner['org_id']}",
