...
class TestGetSprintsByProjectHandler:
    ...
    @pytest.fixture
    def handler(self, sprint_repo) -> GetSprintsByProjectHandler:
        return GetSprintsByProjectHandler(sprint_repo=sprint_repo)

    async def test_get_sprints_with_sprints(self, handler, make_sprint) -> None:
        sprint = await make_sprint()
        query = GetSprintsByProjectQuery(project_id=str(sprint.project_id))
        result = await handler.handle(query)
        assert len(result.items) >= 1

    async def test_get_sprints_empty(self, handler, make_project) -> None:
        project = await make_project()
        query = GetSprintsByProjectQuery(project_id=str(project.id))
        result = await handler.handle(query)
