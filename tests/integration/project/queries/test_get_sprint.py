...
class TestGetSprintHandler:
    ...
    @pytest.fixture
    def handler(self, sprint_repo, permission_checker_stub) -> GetSprintHandler:
        return GetSprintHandler(sprint_repo=sprint_repo, permission_checker=permission_checker_stub)

    async def test_get_sprint_found(self, handler, make_sprint) -> None:
        sprint = await make_sprint(name="Sprint 1")
        query = GetSprintQuery(caller_id=str(Id.generate()), sprint_id=str(sprint.id))
        result = await handler.handle(query)
        assert result.name == "Sprint 1"

    async def test_get_sprint_not_found(self, handler) -> None:
        query = GetSprintQuery(caller_id=str(Id.generate()), sprint_id=str(Id.generate()))
