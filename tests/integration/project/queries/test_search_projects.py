...
class TestSearchProjectsHandler:
    ...
    @pytest.fixture
    def handler(self, project_repo) -> SearchProjectsHandler:
        return SearchProjectsHandler(project_repo=project_repo)

    async def test_search_with_results(self, handler, make_project) -> None:
        await make_project(name="AlphaProject")
        query = SearchProjectsQuery(offset=0, limit=100, filters={"query": "Alpha"})
        result = await handler.handle(query)
        assert result.total >= 1

    async def test_search_empty(self, handler) -> None:
        query = SearchProjectsQuery(offset=0, limit=100, filters={"query": "nonexistent"})
        result = await handler.handle(query)
    ...
    async def test_search_pagination(self, handler, make_project) -> None:
        ...
        query = SearchProjectsQuery(offset=0, limit=1)
        result = await handler.handle(query)
