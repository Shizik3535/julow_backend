...
class TestGetProjectMembersHandler:
    ...
    @pytest.fixture
    def handler(self, membership_repo, permission_checker_stub) -> GetProjectMembersHandler:
        return GetProjectMembersHandler(membership_repo=membership_repo, permission_checker=permission_checker_stub)
    ...
    async def test_get_members_with_members(self, handler, make_project_with_membership) -> None:
        ...
        query = GetProjectMembersQuery(caller_id=str(data["owner_id"]), project_id=str(data["project"].id))
        result = await handler.handle(query)
        assert len(result.items) >= 1

    async def test_get_members_empty(self, handler, make_project) -> None:
        project = await make_project()
        query = GetProjectMembersQuery(caller_id=str(Id.generate()), project_id=str(project.id))
        result = await handler.handle(query)
