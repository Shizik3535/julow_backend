"""Интеграционные тесты AddProjectOwnerHandler (реальные repos + реальные порты)."""
...
class TestAddProjectOwnerHandler:
    """Тесты AddProjectOwnerHandler."""

    @pytest.fixture
    def handler(self, project_repo, permission_checker_stub, event_bus_stub) -> AddProjectOwnerHandler:
        return AddProjectOwnerHandler(
            project_repo=project_repo,
            permission_checker=permission_checker_stub,
            event_bus=event_bus_stub,
        )

    async def test_add_owner_success(self, handler, project_repo, make_project, make_user) -> None:
        owner = await make_user()
        co_owner = await make_user()
        project = await make_project(owner_id=owner.id)

        cmd = AddProjectOwnerCommand(
        ...
        assert co_owner.id in found.owner_ids
