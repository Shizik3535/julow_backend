...
class TestCreateProjectRoleHandler:
    ...
    @pytest.fixture
    def handler(self, role_repo, permission_checker_stub, event_bus_stub) -> CreateProjectRoleHandler:
    ...
    async def test_create_role_success(self, handler, role_repo, make_project) -> None:
        ...
        cmd = CreateProjectRoleCommand(
            ...
            name="Reviewer",
            permissions=["content.read", "tasks.read"],
        )
        result = await handler.handle(cmd)

        assert result.name == "Reviewer"
        assert "content.read" in result.permissions

        role = await role_repo.get_by_id(Id.from_string(result.id))
        assert role is not None

    async def test_create_role_insufficient_permission(self, role_repo, event_bus_stub) -> None:
