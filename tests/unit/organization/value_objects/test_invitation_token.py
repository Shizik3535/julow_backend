"""Unit-тесты для InvitationToken (Organization BC)."""
...
class TestInvitationToken:
    ...
    def test_max_uses_less_than_one_raises(self) -> None:
        with pytest.raises(ValidationException) as exc_info:
    ...
    def test_is_max_uses_exceeded_false_when_no_limit(self) -> None:
        token = InvitationToken(value="abc", used_count=10)
    ...
    def test_equality_by_value(self) -> None:
        t1 = InvitationToken(value="abc", max_uses=5, used_count=0)
        t2 = InvitationToken(value="abc", max_uses=5, used_count=0)
        assert t1 == t2
