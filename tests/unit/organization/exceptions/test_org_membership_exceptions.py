"""Unit-тесты для исключений OrgMembership aggregate (Organization BC)."""
...
@pytest.mark.unit
class TestOrgMemberNotFoundException:
    def test_is_entity_not_found(self) -> None:
        exc = OrgMemberNotFoundException(id="user-1")
        assert isinstance(exc, EntityNotFoundException)
        assert exc.entity_type == "OrgMember"
...
class TestMembershipLimitExceededException:
    ...
    def test_rule(self) -> None:
        exc = MembershipLimitExceededException()
...
class TestEmailDomainNotAllowedException:
    ...
    def test_rule(self) -> None:
        exc = EmailDomainNotAllowedException()
