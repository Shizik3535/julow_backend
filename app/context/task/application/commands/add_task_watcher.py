...
class AddTaskWatcherCommand(BaseCommand):
    """
    Команда добавления наблюдателя задачи.

    Атрибуты:
        caller_id: ID пользователя, выполняющего операцию.
        task_id: ID задачи.
        user_id: ID пользователя, которого назначают наблюдателем.
    """
...
class AddTaskWatcherHandler(BaseCommandHandler[AddTaskWatcherCommand, None]):
    """Обработчик добавления наблюдателя задачи."""
    ...
    async def handle(self, command: AddTaskWatcherCommand) -> None:
        ...
        task.add_watcher(Id.from_string(command.user_id))
