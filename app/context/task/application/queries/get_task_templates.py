...
class GetTaskTemplatesQuery(BaseQuery):
    """Запрос системных шаблонов задач."""

    pass


class GetTaskTemplatesHandler(BaseQueryHandler[GetTaskTemplatesQuery, TaskTemplateListDTO]):
    """Обработчик получения системных шаблонов задач."""

    def __init__(self, template_repo: TaskTemplateRepository) -> None:
        super().__init__()
        self._template_repo = template_repo

    async def handle(self, query: GetTaskTemplatesQuery) -> TaskTemplateListDTO:
        templates = await self._template_repo.get_system_templates()
        items = [
            TaskTemplateDTO(
                id=str(t.id),
                name=t.name,
