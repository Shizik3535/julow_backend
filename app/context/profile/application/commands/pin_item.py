...
class PinItemCommand(BaseCommand):
    """
    Команда закрепления элемента.
...
class PinItemHandler(BaseCommandHandler[PinItemCommand, None]):
    """
    Обработчик закрепления элемента.

    Загружает профиль, вызывает pin_item, сохраняет.
    """
    ...
    async def handle(self, command: PinItemCommand) -> None:
