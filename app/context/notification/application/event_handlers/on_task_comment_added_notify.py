...
class OnTaskCommentAddedNotify(BaseEventHandler[dict[str, Any]]):
    """
    Обработчик события TaskCommentAdded из Task BC.

    Создаёт task_comment уведомление для каждого участника задачи.
    """
    ...
    async def handle(self, event: dict[str, Any]) -> None:
        ...
        if event_type != "TaskCommentAdded":
            return
        ...
        # Не уведомлять автора комментария
        recipients = [uid for uid in participant_ids if uid != author_id]
