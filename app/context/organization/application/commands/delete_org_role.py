...
class DeleteOrgRoleCommand(BaseCommand):
    """
    Команда удаления кастомной роли.
...
class DeleteOrgRoleHandler(BaseCommandHandler[DeleteOrgRoleCommand, None]):
    """
    Обработчик удаления роли.

    Загружает OrgRole, проверяет что не системная, помечает удалённой.
    Домен гарантирует, что системные роли нельзя удалить.
    """
    ...
    async def handle(self, command: DeleteOrgRoleCommand) -> None:
