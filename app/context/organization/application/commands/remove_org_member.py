...
class RemoveOrgMemberCommand(BaseCommand):
    """
...
class RemoveOrgMemberHandler(BaseCommandHandler[RemoveOrgMemberCommand, None]):
    """
    Обработчик удаления участника из организации.

    Проверяет, является ли пользователь владельцем (через Organization AR),
    затем удаляет из OrgMembership.
    """
    ...
    async def handle(self, command: RemoveOrgMemberCommand) -> None:
