...
class UpdateDepartmentCommand(BaseCommand):
    """
...
class UpdateDepartmentHandler(BaseCommandHandler[UpdateDepartmentCommand, None]):
    """
    Обработчик обновления подразделения.

    Загружает Department, вызывает update, сохраняет.
    """
    ...
    async def handle(self, command: UpdateDepartmentCommand) -> None:
