...
class RemoveRoleCommand(BaseCommand):
    """
    Команда снятия роли с пользователя.

    Атрибуты:
        caller_id: Идентификатор пользователя, выполняющего операцию.
        user_id: Идентификатор пользователя, с которого снимается роль.
...
class RemoveRoleHandler(BaseCommandHandler[RemoveRoleCommand, None]):
    """
    Обработчик снятия роли.

    Проверяет разрешение caller'а, загружает User и Role,
    снимает роль, сохраняет.
    """
    ...
    async def handle(self, command: RemoveRoleCommand) -> None:
        ...
        user.remove_role(role_id, is_system_role=role.is_system)
