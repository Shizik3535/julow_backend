from __future__ import annotations

from app.shared.application.base_command import BaseCommand
from app.shared.application.base_command_handler import BaseCommandHandler
from app.shared.application.messaging.domain_event_bus import DomainEventBus
from app.shared.domain.value_objects.id_vo import Id
from app.context.identity.domain.exceptions.user_exceptions import UserNotFoundException
from app.context.identity.domain.repositories.user_repository import UserRepository


class DisableAccountCommand(BaseCommand):
    """
    Команда деактивации аккаунта.

    Атрибуты:
        user_id: Идентификатор пользователя.
    """

    user_id: str


class DisableAccountHandler(BaseCommandHandler[DisableAccountCommand, None]):
    """
    Обработчик деактивации аккаунта.

    Загружает User, деактивирует, сохраняет.
    """

    def __init__(
        self,
        user_repo: UserRepository,
        event_bus: DomainEventBus,
    ) -> None:
        super().__init__()
        self._user_repo = user_repo
        self._event_bus = event_bus

    async def handle(self, command: DisableAccountCommand) -> None:
        user_id = Id.from_string(command.user_id)

        user = await self._user_repo.get_by_id(user_id)
        if user is None:
            raise UserNotFoundException(command.user_id)

        user.disable()

        await self._user_repo.update(user)
        await self._event_bus.publish_all(user.clear_domain_events())
