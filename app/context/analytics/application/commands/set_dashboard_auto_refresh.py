...
class SetDashboardAutoRefreshCommand(BaseCommand):
    """Настройка авто-обновления дашборда.

    При enabled=True interval_seconds обязателен (>= 30) — иначе ValidationException.
    """
...
class SetDashboardAutoRefreshHandler(
    BaseCommandHandler[SetDashboardAutoRefreshCommand, DashboardDTO]
):
    ...
    async def handle(self, command: SetDashboardAutoRefreshCommand) -> DashboardDTO:
        ...
        dashboard.set_auto_refresh(command.enabled, command.interval_seconds)
