...
class InvitationToken(ValueObject):
    """
    Value Object для токена ссылки-приглашения в проект.

    Один и тот же токен может выступать как «ссылкой», так и
    «коротким кодом» (UI просто отображает первые символы,
    но проверяется именно полное значение).
