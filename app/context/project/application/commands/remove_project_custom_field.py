...
class RemoveProjectCustomFieldCommand(BaseCommand):
    """
    Команда удаления кастомного поля проекта.
...
class RemoveProjectCustomFieldHandler(BaseCommandHandler[RemoveProjectCustomFieldCommand, None]):
    """Обработчик удаления кастомного поля проекта."""
    ...
    async def handle(self, command: RemoveProjectCustomFieldCommand) -> None:
        ...
        project.remove_custom_field(command.name)
