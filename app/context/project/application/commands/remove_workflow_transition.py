...
class RemoveWorkflowTransitionCommand(BaseCommand):
    """
    Команда удаления перехода workflow.
...
class RemoveWorkflowTransitionHandler(BaseCommandHandler[RemoveWorkflowTransitionCommand, None]):
    """Обработчик удаления перехода workflow."""
    ...
    async def handle(self, command: RemoveWorkflowTransitionCommand) -> None:
        ...
        board.remove_workflow_transition(Id.from_string(command.transition_id))
