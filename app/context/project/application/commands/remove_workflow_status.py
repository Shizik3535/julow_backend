...
class RemoveWorkflowStatusCommand(BaseCommand):
    """
    Команда удаления статуса workflow.
...
class RemoveWorkflowStatusHandler(BaseCommandHandler[RemoveWorkflowStatusCommand, None]):
    """Обработчик удаления статуса workflow."""
    ...
    async def handle(self, command: RemoveWorkflowStatusCommand) -> None:
        ...
        board.remove_workflow_status(Id.from_string(command.status_id))
