...
class AddProjectMilestoneCommand(BaseCommand):
    """
    Команда добавления milestone в проект.
...
class AddProjectMilestoneHandler(BaseCommandHandler[AddProjectMilestoneCommand, MilestoneDTO]):
    """
    Обработчик добавления milestone.

    Проверяет capabilities.has_milestones.
    """
    ...
    async def handle(self, command: AddProjectMilestoneCommand) -> MilestoneDTO:
        ...
        return MilestoneDTO(
            id=str(milestone.id),
            name=milestone.name,
            description={"content": description.content, "format": description.format.value} if description else None,
