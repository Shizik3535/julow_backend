...
class RemoveBoardColumnCommand(BaseCommand):
    """
    Команда удаления колонки с доски.
...
class RemoveBoardColumnHandler(BaseCommandHandler[RemoveBoardColumnCommand, None]):
    """Обработчик удаления колонки с доски."""
    ...
    async def handle(self, command: RemoveBoardColumnCommand) -> None:
        ...
        board.remove_column(Id.from_string(command.column_id))
