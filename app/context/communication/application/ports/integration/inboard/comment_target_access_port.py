...
class CommentTargetAccessPort(ABC):
    ...
    async def resolve_project_id(
        ...
        """
        Найти ``project_id``, к которому относится комментируемая сущность.

        Используется для маркировки вложений комментариев тегом
        ``project:<id>``, чтобы UI документов мог группировать файлы по
        проекту независимо от того, в какой папке они физически лежат.

        Возвращает ``None``, если у цели нет project (например, для
        неподдерживаемых ``target_type``).
        """
