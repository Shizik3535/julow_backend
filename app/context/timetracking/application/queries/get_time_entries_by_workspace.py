...
class GetTimeEntriesByWorkspaceQuery(BaseQuery):
    """Запрос: записи времени в workspace."""

    caller_id: str
    workspace_id: str


class GetTimeEntriesByWorkspaceHandler(
    BaseQueryHandler[GetTimeEntriesByWorkspaceQuery, TimeEntryListDTO]
):
    """Записи в workspace. Требует time.read."""
    ...
    async def handle(self, query: GetTimeEntriesByWorkspaceQuery) -> TimeEntryListDTO:
        await self._permission_checker.require_permission(
            user_id=query.caller_id,
            workspace_id=query.workspace_id,
            permission=self.REQUIRED_PERMISSION,
        )
        entries = await self._repo.get_by_workspace(Id.from_string(query.workspace_id))
