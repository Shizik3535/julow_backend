...
class ResumeTimerCommand(BaseCommand):
    """Команда возобновления таймера после паузы."""

    caller_id: str
    entry_id: str


class ResumeTimerHandler(BaseCommandHandler[ResumeTimerCommand, TimeEntryDTO]):
    """Возобновление таймера. Доступно только владельцу записи."""
    ...
    async def handle(self, command: ResumeTimerCommand) -> TimeEntryDTO:
