...
class RejectTimeEntryCommand(BaseCommand):
    """Команда отклонения записи времени (менеджер)."""
...
class RejectTimeEntryHandler(BaseCommandHandler[RejectTimeEntryCommand, TimeEntryDTO]):
    """SUBMITTED → REJECTED."""
    ...
    async def handle(self, command: RejectTimeEntryCommand) -> TimeEntryDTO:
        ...
        entry.reject(rejected_by=Id.from_string(command.caller_id), reason=command.reason)
