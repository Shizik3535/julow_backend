...
class PauseTimerCommand(BaseCommand):
    """Команда паузы таймера."""

    caller_id: str
    entry_id: str


class PauseTimerHandler(BaseCommandHandler[PauseTimerCommand, TimeEntryDTO]):
    """Пауза таймера. Доступна только владельцу записи."""
    ...
    async def handle(self, command: PauseTimerCommand) -> TimeEntryDTO:
