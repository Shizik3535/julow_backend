...
class ResubmitTimeEntryCommand(BaseCommand):
    """Команда повторной отправки отклонённой записи на утверждение."""

    caller_id: str
    entry_id: str


class ResubmitTimeEntryHandler(BaseCommandHandler[ResubmitTimeEntryCommand, TimeEntryDTO]):
    """REJECTED → SUBMITTED. Только владелец."""

    REQUIRED_PERMISSION = "time.submit"
    ...
    async def handle(self, command: ResubmitTimeEntryCommand) -> TimeEntryDTO:
