...
class ApproveTimeEntryCommand(BaseCommand):
    """Команда утверждения записи времени (менеджер)."""

    caller_id: str
    entry_id: str


class ApproveTimeEntryHandler(BaseCommandHandler[ApproveTimeEntryCommand, TimeEntryDTO]):
    """SUBMITTED → APPROVED. Нельзя утверждать свою же запись."""
    ...
    async def handle(self, command: ApproveTimeEntryCommand) -> TimeEntryDTO:
        ...
        entry.approve(approved_by=Id.from_string(command.caller_id))
