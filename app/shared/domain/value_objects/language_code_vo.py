...
class LanguageCode(ValueObject):
    """
    Value Object для кода языка (ISO 639-1).

    Гарантирует, что значение является валидным двухбуквенным кодом
    языка согласно ISO 639-1.
    Хранится в нижнем регистре для единообразия.

    Атрибуты:
        value: Двухбуквенный код языка (например "ru", "en").
    ...
    def __post_init__(self) -> None:
        ...
        if normalized not in _ISO_639_1_CODES:
            raise ValidationException(
                field="language_code",
                message=f"Неизвестный код языка (ISO 639-1): {normalized}",
            )
