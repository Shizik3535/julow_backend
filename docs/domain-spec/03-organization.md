...
| VO | Тип | Описание | Исходное требование |
|---|---|---|---|
| `OrgPersonalization` | frozen dataclass | color: AccentColor, icon: str \| None, display_name: str \| None, custom_domain: str \| None, branding: OrgBranding \| None | §3.4 |
...
| `admin` | `org.read`, `org.settings.*`, `members.*`, `roles.*`, `teams.*`, `departments.*`, `invitations.read`, `workspaces.*` | ORG | Управление организацией |
| `moderator` | `org.read`, `members.read`, `members.invite`, `invitations.read`, `teams.*`, `departments.read`, `workspaces.read`, `workspaces.members.read`, `workspaces.members.invite`, `workspaces.projects.read`, `workspaces.projects.tasks.*` | ORG | Ограниченное управление |
| `member` | `org.read`, `members.read`, `teams.read`, `workspaces.read`, `workspaces.projects.read`, `workspaces.projects.tasks.read` | ORG | Базовый доступ |
